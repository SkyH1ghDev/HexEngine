...
public:
    ...
    static MW::ComPtr<IDXGISwapChain4> CreateSwapChain(const SDLWindow& window, const MW::ComPtr<ID3D12CommandQueue>& commandQueue, const std::uint32_t& width, const std::uint32_t& height, const std::uint32_t& bufferCount);
    ...
    static std::vector<MW::ComPtr<ID3D12Resource>> CreateBackBuffers(const MW::ComPtr<ID3D12Device2>& device, const MW::ComPtr<IDXGISwapChain4>& swapChain, const MW::ComPtr<ID3D12DescriptorHeap>& descriptorHeap, const std::uint8_t& numBuffers, UINT& rtvDescriptorSize);
